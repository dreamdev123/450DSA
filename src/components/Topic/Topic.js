  ...
  */
	...
	const onHandleDone = (num, answer) => {
		...
		let updatedQuestionsStatus = data.questions.map((question, index) => {
			if (num === index) {
				...
				if (isSelect) {
					newDoneQuestion.push(num);
				} else {
					var pos = newDoneQuestion.indexOf(num);
		...
		if (isSelectedComplete) displayToast(isSelect, num);
	}
						...
						<div style={{ display: 'flex', justifyContent: 'space-between' }}>
							<div className='container'>
								...
								<ul style={{ "listStyle": "none" }}>
									{question.option.map((op, i) => {
										return question.Done ? (<li className={i == question.selected ? (i == question.answer ? "True_question" : "False_question") : i == question.answer ? "True_question" : ""}><input type='checkbox' className='mr-2' checked={i == question.selected ? true : false} onChange={(e) => {
											console.log(e.target.checked, index, i);
											onHandleDone(index, i);
										}} disabled={question.Done} />{op}</li>)
											: (<li><input type='checkbox' className='mr-2' checked={i == question.selected ? true : false} onChange={(e) => {
												onHandleDone(index, i);
											}} disabled={question.Done} />{op}</li>);
									})}
								</ul>
	...
	const columns = [
		...
		},
		{
			dataField: 'question',
			text: 'Questions',
			headerStyle: { fontSize: '20px', textAlign: 'center', width: '85%' },
		},
		{
			dataField: 'controls',
			text: '',
	...
	function shownotes(ind) {
		...
		sessionStorage.setItem('cid', ind);
					...
					{(props) => (
						<div>
							...
							<div className='container container-custom' style={{ overflowAnchor: 'none' }}>
								<Fade duration={600}>
									<BootstrapTable {...props.baseProps}  sort={sortMode} classes={dark ? 'dark-table' : ''} />
								</Fade>
							</div>
						</div>
					)}
